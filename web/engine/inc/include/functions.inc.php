...
function check_login($username, $md5_password, $post = true, $check_log = false) {
	...
	if ( !$result ) { 
		...
		if ( $check_log AND !$_SESSION['check_log']) {
			...
			$db->query( "INSERT INTO " . USERPREFIX . "_admin_logs (name, date, ip, action, extras) values ('".$db->safesql($member_id['name'])."', '{$_TIME}', '{$_IP}', '82', '')" );
...
function build_js($js) {
	...
	if ($config['js_min'] AND version_compare(PHP_VERSION, '5.1.0', '>') ) {
		...
		$js_array[] = "<script type=\"text/javascript\" src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;g=admin&amp;5\"></script>";

		if ( count($js) ) $js_array[] = "<script type=\"text/javascript\" src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $js)."&amp;5\"></script>";
