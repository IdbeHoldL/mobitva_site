...
HTML;
    ...
    <tr>
        ...
        <td style="padding:2px;"><input type="checkbox" name="allow_count" value="1" checked> {$lang['allow_count']}</td>
    </tr>
    <tr>
        <td style="padding:2px;">&nbsp;</td>
...
HTML;
  ...
  // Обработка даты и времени
	...
	$db->query( "INSERT INTO " . PREFIX . "_static (name, descr, template, allow_br, allow_template, grouplevel, tpl, metadescr, metakeys, template_folder, date, metatitle, allow_count) values ('$name', '$descr', '$template', '$allow_br', '$allow_template', '$grouplevel', '$tpl', '{$metatags['description']}', '{$metatags['keywords']}', '{$skin_name}', '{$thistime}', '{$metatags['title']}', '$allow_count')" );
	$row = $db->insert_id();
...
HTML;
    ...
    <tr>
        <td style="padding:2px;">&nbsp;</td>
        <td style="padding-left:2px;"><input type="checkbox" name="allow_count" value="1" {$check_c}> {$lang['allow_count']}</td>
    </tr>
    <tr>
        <td style="padding:2px;">&nbsp;</td>
...
HTML;
	...
	if( $_GET['page'] == "rules" ) {
		...
		if( $_POST['id'] ) {
			...
			$db->query( "INSERT INTO " . PREFIX . "_static (name, descr, template, allow_br, allow_template, grouplevel, tpl, metadescr, metakeys, template_folder, date, metatitle, allow_count) values ('$name', '$descr', '$template', '$allow_br', '$allow_template', 'all', '$tpl', '{$metatags['description']}', '{$metatags['keywords']}', '{$skin_name}', '{$thistime}', '{$metatags['title']}', '{$allow_count}')" );
			$row = $db->insert_id();
			$db->query( "UPDATE " . PREFIX . "_static_files SET static_id='{$row}' WHERE author = '{$member_id['name']}' AND static_id = '0'" );
