...
<script type="text/javascript">
    ...
    <tr>
        ...
        <td><select name="category[]" id="category" onchange="onCategoryChange(this)" {$category_multiple}>
		{$categories_list}
		</select>
		</td>
    </tr>
...
elseif( $action == "doeditnews" ) {
	...
	if ($config['create_catalog'] AND !$catalog_url) $catalog_url = totranslit($title{0});
	...
	if( $item_db[0] ) {
		...
		if( $ifdelete == "yes" ) {
			...
			foreach ( $item_db[5] as $selected ) {
				if( $allow_list[0] != "all" AND !in_array($selected, $allow_list) AND $item_db[3]) $have_perm = 0;
			}
