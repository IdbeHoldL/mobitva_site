			...
			foreach ($xfields as $name => $value) {
				...
				if ($value[3] == "textarea" AND $postedxfields[$value[0]] != "" ) {
					...
					if( $config['allow_admin_wysiwyg'] == "yes" or $row['allow_br'] != '1' ) {
						...
						$postedxfields[$value[0]] = $parsexf->BB_Parse($parsexf->process($postedxfields[$value[0]]));
							
					} else {
						$postedxfields[$value[0]] = $parsexf->decodeBBCodes($postedxfields[$value[0]], false);
						$postedxfields[$value[0]] = $parsexf->BB_Parse($parsexf->process($postedxfields[$value[0]]), false);
							
					}
			
				} elseif ( $postedxfields[$value[0]] != "" ) {
			
					$postedxfields[$value[0]] = $parsexf->process(stripslashes($postedxfields[$value[0]]));
