	...
	while ( $row = $db->get_row( $sql_result ) ) {
		...
		if( $config['related_news'] ) {
			...
			if( $buffer === FALSE ) {
				...
				$db->query( "SELECT id, short_story, title, date, category, alt_name, flag FROM " . PREFIX . "_post WHERE MATCH (title, short_story, full_story, xfields) AGAINST ('$body') AND id != " . $row['id'] . " AND approve=1" . $where_date . " LIMIT " . $config['related_number'] );
				...
				while ( $related = $db->get_row() ) {
					...
					$tpl2->set( '{title}', htmlspecialchars( strip_tags( stripslashes( $related['title'] ) ) ) );
		...
		else $tpl->result['content'] = preg_replace ( "#\[hide\](.+?)\[/hide\]#ims", "<div class=\"quote\">" . $lang['news_regus'] . "</div>  <div style='display:none;'>dle и <a href='http://dlebest.ru' >шаблоны DlE</a> </div>", $tpl->result['content'] );
